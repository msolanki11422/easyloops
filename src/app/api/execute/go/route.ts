...
export async function POST(request: Request) {
  try {
    ...
    console.log("API received code:", code.substring(0, 100) + "...");
    ...
    console.log("Wrote code to file:", goFilePath);

    // If input is provided, write it to a file
    let command = `cd ${tempDir} && go run main.go`;
    if (input) {
      const inputFilePath = path.join(tempDir, "input.txt");
      await fs.writeFile(inputFilePath, input);
      command = `cd ${tempDir} && cat input.txt | go run main.go`;
      console.log("Wrote input to file:", inputFilePath);
    }

    console.log("Executing command:", command);

    // Run the Go code using the local Go compiler
